...
export default function GeneratePage() {
  ...
  const callVolcengineAPI = async () => {
    ...
    const petDescription = [
      config.petName ? `named ${config.petName}` : '',
      config.breed || '',
      config.personality || ''
    ].filter(Boolean).join(', ')

    const prompt = buildPrompt(config, petDescription)

    const volcengineSize = sizeMap[String(config.size)] || '2k'

    setApiDebugInfo(`config.size: ${config.size}, mapped size: ${volcengineSize}`)
