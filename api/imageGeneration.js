...
const stylePrompts = {
  cute: (pet) =>
    `A cute chibi-style cartoon illustration of ${pet}, with big sparkling eyes, small body, kawaii aesthetic, soft pastel colors, adorable expression, high quality digital art`,
  realistic: (pet) =>
...
export function buildPrompt(config, petDescription = '') {
  const pet = petDescription || 'a lovely pet'
  const styleMap = {
    cartoon: 'cute',
    artistic: 'watercolor',
    '3d': 'realistic',
  ...
  const styleKey = styleMap[config.style] || 'realistic'
  const basePrompt = stylePrompts[styleKey](pet)

  if (config.background === 'transparent') {
    return `${basePrompt}, isolated on transparent background, no background`
  } else if (config.background === 'white') {
    return `${basePrompt}, clean white background`
  } else if (config.background === 'gradient') {
    return `${basePrompt}, beautiful gradient background with soft colors`
  }
